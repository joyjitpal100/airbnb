// Guest ID Vault - Netlify Function
// All routes: /api/vault/*
  ...
  try {
    ...
    if (path === '/guests' && method === 'GET') {
      ...
      if (params.property_name) {
        query = query.eq('property_name', params.property_name);
      }
      if (params.booking_source) {
        query = query.eq('booking_source', params.booking_source);
      }
      if (params.status) {
    ...
    if (path === '/guests' && method === 'POST') {
      ...
      if (!fields.guest_name) {
        return errorResponse('Guest name is required');
      }
      if (!fields.check_in_date && !fields.check_out_date) {
        return errorResponse('At least one date required');
      }
      ...
      if (file && file.buffer.length > 0) {
        ...
        if (!ALLOWED_EXTENSIONS.includes(ext)) {
          return errorResponse('Invalid file type. Allowed: jpg, jpeg, png, pdf');
        }
        ...
        file_path = `documents/${uniqueId}${ext}`;
        
        await uploadToR2(file.buffer, file_path, file.mimeType);
      }
      
      const now = new Date().toISOString();
      const retention_until = calculateRetentionDate(fields.check_out_date, now);
    ...
    // GET /guests/:id - Get single record
    ...
    // GET /guests/:id/document-url - Get presigned URL
    ...
    // PATCH /guests/:id/soft-delete - Soft delete
    ...
    if (permDelMatch && method === 'DELETE') {
      ...
      // Delete from R2
      if (record.file_path) {
        await deleteFromR2(record.file_path);
      }
    ...
    // GET /retention-review - Retention dashboard
    if (path === '/retention-review' && method === 'GET') {
      const today = new Date();
      const todayStr = today.toISOString().split('T')[0];
      const thirtyDays = new Date(today.setDate(today.getDate() + 30)).toISOString().split('T')[0];
    ...
    // GET /properties - Unique properties
